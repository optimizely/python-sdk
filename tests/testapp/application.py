...
def before_request():
    ...
    if user_profile_service_instance:
        ups_class = getattr(
            user_profile_service, request.payload.get("user_profile_service")
        )
...
def activate():
    ...
    variation = optimizely_instance.activate(
        experiment_key, user_id, attributes=attributes
    )
    user_profiles = (
        user_profile_service_instance.user_profiles.values()
        if user_profile_service_instance
        else []
    )
...
def get_variation():
    ...
    user_id = payload.get("user_id")
    attributes = payload.get("attributes")
    variation = optimizely_instance.get_variation(
        experiment_key, user_id, attributes=attributes
    )
    user_profiles = (
        user_profile_service_instance.user_profiles.values()
...
def is_feature_enabled():
    ...
    feature_enabled = optimizely_instance.is_feature_enabled(
        feature_flag_key, user_id, attributes
    )
    user_profiles = (
        user_profile_service_instance.user_profiles.values()
        if user_profile_service_instance
    ...
    result = (
        ...
        else "true" if feature_enabled is True else "false"
    )
    return (
...
def get_feature_variable_double():
    ...
    double_value = optimizely_instance.get_feature_variable_double(
        feature_flag_key, variable_key, user_id, attributes
    )
...
def get_feature_variable_string():
    ...
    string_value = optimizely_instance.get_feature_variable_string(
        feature_flag_key, variable_key, user_id, attributes
    )
...
def forced_variation():
    ...
    result = optimizely_instance.set_forced_variation(
        experiment_key, user_id, forced_variation_key
    )
    if result is False:
        return (
...
def forced_variation_multiple_sets():
    ...
    result = optimizely_instance.set_forced_variation(
        experiment_key_1, user_id_1, forced_variation_key_1
    )
    if result is False:
        return (
            json.dumps({"result": None, "user_profiles": user_profiles}),
    ...
    result = optimizely_instance.set_forced_variation(
        experiment_key_2, user_id_1, forced_variation_key_2
    )
    if result is False:
        return (
            json.dumps({"result": None, "user_profiles": user_profiles}),
    ...
    result = optimizely_instance.set_forced_variation(
        experiment_key_1, user_id_2, forced_variation_key_1
    )
    if result is False:
        return (
            json.dumps({"result": None, "user_profiles": user_profiles}),
    ...
    result = optimizely_instance.set_forced_variation(
        experiment_key_2, user_id_2, forced_variation_key_2
    )
    if result is False:
        return (
            json.dumps({"result": None, "user_profiles": user_profiles}),
