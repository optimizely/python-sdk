...
class HoldoutConfigTest(base.BaseTest):
    def setUp(self):
        ...
        # Rule IDs from the test config experiments
    ...
    def test_get_global_holdouts__returns_global_holdouts(self):
        ...
        global_holdout = holdouts[0]
        self.assertEqual('holdout_1', global_holdout.id)
        self.assertEqual('global_holdout', global_holdout.key)
    ...
    def test_get_holdouts_for_rule__returns_local_holdouts(self):
        ...
        holdouts = self.config_with_holdouts.get_holdouts_for_rule('111127')
        self.assertEqual(1, len(holdouts))
        self.assertEqual('specific_holdout', holdouts[0].key)
        self.assertFalse(holdouts[0].is_global)
    ...
    def test_holdout_initialization__categorizes_holdouts_properly(self):
        ...
        # holdout_1 is global (no includedRules)
        ...
        self.assertNotIn('holdout_2', holdout_ids_in_global)

        # holdout_2 is local, mapped to rule '111127'
        self.assertIn('111127', self.config_with_holdouts.rule_holdouts_map)
        rule_holdout_ids = [h.id for h in self.config_with_holdouts.rule_holdouts_map['111127']]
        self.assertIn('holdout_2', rule_holdout_ids)
    ...
    def test_holdout_entity__is_global_property(self):
        ...
        empty_local_holdout = entities.Holdout(
            id='3', key='empty_local', status='Running',
            variations=[], trafficAllocation=[], audienceIds=[],
            includedRules=[]
        )
        self.assertFalse(empty_local_holdout.is_global)

    def test_holdout__none_vs_empty_array_distinction(self):
        ...
        self.assertEqual(1, len(global_holdouts))
        self.assertIsNone(global_holdouts[0].includedRules)

        # Empty local holdout should not appear in global list or rule maps
        empty_holdout = entities.Holdout(
            id='empty', key='empty', status='Running',
            variations=[], trafficAllocation=[], audienceIds=[],
            includedRules=[]
        )
        self.assertFalse(empty_holdout.is_global)


class LocalHoldoutConfigTest(base.BaseTest):
    """ Tests for local holdout (rule-level targeting) config. """
        ...
        config_dict['holdouts'] = [
            {
                ...
                'audienceIds': [],
                'variations': [
                    {'id': 'global_var_1', 'key': 'global_control', 'variables': []}
                ],
                'trafficAllocation': [
                    {'entityId': 'global_var_1', 'endOfRange': 5000}
                ]
            },
            {
                'id': 'local_holdout_1',
                ...
                'audienceIds': [],
                'variations': [
                    {'id': 'local_var_1', 'key': 'local_control', 'variables': []}
                ],
                'trafficAllocation': [
                    {'entityId': 'local_var_1', 'endOfRange': 5000}
                ]
            },
            {
                'id': 'local_holdout_2',
                ...
                'audienceIds': [],
                'variations': [
                    {'id': 'local_var_2', 'key': 'local_multi_control', 'variables': []}
                ],
                'trafficAllocation': [
                    {'entityId': 'local_var_2', 'endOfRange': 5000}
                ]
            },
            {
                ...
                'audienceIds': [],
                'variations': [
                    {'id': 'local_var_empty', 'key': 'empty_control', 'variables': []}
                ],
                'trafficAllocation': [
                    {'entityId': 'local_var_empty', 'endOfRange': 10000}
                ]
            },
            {
                'id': 'draft_holdout',
                ...
                'audienceIds': [],
                'variations': [
                    {'id': 'draft_var', 'key': 'draft_control', 'variables': []}
                ],
                'trafficAllocation': [
                    {'entityId': 'draft_var', 'endOfRange': 10000}
                ]
            },
            {
                ...
                'audienceIds': [],
                'variations': [
                    {'id': 'local_var_invalid', 'key': 'invalid_control', 'variables': []}
                ],
                'trafficAllocation': [
                    {'entityId': 'local_var_invalid', 'endOfRange': 10000}
                ]
            }
        ]
    ...
    def test_global_holdouts_correctly_identified(self):
        """ Test that global holdouts are correctly identified. """

        global_holdouts = self.project_config.get_global_holdouts()
        self.assertEqual(1, len(global_holdouts))
        self.assertEqual('global_holdout', global_holdouts[0].key)
        self.assertTrue(global_holdouts[0].is_global)

    def test_rule_holdouts_map(self):
        """ Test that local holdouts are correctly mapped to rules. """

        # Rule 1 should have 2 local holdouts
        holdouts_for_rule_1 = self.project_config.get_holdouts_for_rule('111127')
        self.assertEqual(2, len(holdouts_for_rule_1))
        ...
        holdouts_for_rule_2 = self.project_config.get_holdouts_for_rule('32222')
        self.assertEqual(1, len(holdouts_for_rule_2))
        self.assertEqual('local_holdout_multi', holdouts_for_rule_2[0].key)

        # Non-existent rule should return empty list
        self.assertEqual(0, len(self.project_config.get_holdouts_for_rule('non_existent')))

    def test_empty_included_rules_not_mapped(self):
        """ Test that holdouts with empty includedRules are not mapped to any rules. """
    ...
    def test_draft_holdouts_not_processed(self):
        ...
        global_keys = {h.key for h in self.project_config.get_global_holdouts()}
        self.assertNotIn('draft_holdout', global_keys)

        rule_keys = {h.key for h in self.project_config.get_holdouts_for_rule('111127')}
    ...
    def test_invalid_rule_ids_handled_silently(self):
        ...
        # The holdout entity still exists, just not mapped
        invalid_holdout = self.project_config.get_holdout('local_holdout_invalid')
        self.assertIsNotNone(invalid_holdout)
        self.assertEqual('local_holdout_invalid_rule', invalid_holdout.key)

    def test_cross_rule_targeting(self):
        """ Test that a single holdout can target rules from multiple experiments. """

        holdouts_1 = self.project_config.get_holdouts_for_rule('111127')
        holdouts_2 = self.project_config.get_holdouts_for_rule('32222')
        ...
        # Same holdout object
        multi_1 = next(h for h in holdouts_1 if h.key == 'local_holdout_multi')
        multi_2 = next(h for h in holdouts_2 if h.key == 'local_holdout_multi')
        self.assertEqual(multi_1.id, multi_2.id)
