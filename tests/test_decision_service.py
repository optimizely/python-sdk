...
class DecisionServiceTest(base.BaseTest):
    ...
    def test_get_bucketing_id__no_bucketing_id_attribute(self):
        ...
        self.assertEqual(
            'test_user', self.decision_service._get_bucketing_id('test_user', {'random_key': 'random_value'}),
        )
    ...
    def test_get_bucketing_id__bucketing_id_attribute(self):
        ...
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging:
            self.assertEqual(
                'user_bucket_value',
                self.decision_service._get_bucketing_id('test_user', {'$opt_bucketing_id': 'user_bucket_value'}),
            )
    ...
    def test_get_bucketing_id__bucketing_id_attribute_not_a_string(self):
        ...
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging:
            self.assertEqual(
                'test_user', self.decision_service._get_bucketing_id('test_user', {'$opt_bucketing_id': True}),
            )
            ...
            self.assertEqual(
                'test_user', self.decision_service._get_bucketing_id('test_user', {'$opt_bucketing_id': 5.9}),
            )
            mock_decision_service_logging.warning.assert_called_once_with(
                'Bucketing ID attribute is not a string. Defaulted to user_id.'
            )
            ...
            self.assertEqual(
                'test_user', self.decision_service._get_bucketing_id('test_user', {'$opt_bucketing_id': 5}),
            )
            mock_decision_service_logging.warning.assert_called_once_with(
                'Bucketing ID attribute is not a string. Defaulted to user_id.'
            )
    ...
    def test_set_forced_variation__invalid_experiment_key(self):
        ...
        self.assertFalse(
            self.decision_service.set_forced_variation(
                self.project_config, 'test_experiment_not_in_datafile', 'test_user', 'variation',
            )
        )
        self.assertFalse(self.decision_service.set_forced_variation(self.project_config, '', 'test_user', 'variation'))
        self.assertFalse(
            self.decision_service.set_forced_variation(self.project_config, None, 'test_user', 'variation')
        )
    ...
    def test_set_forced_variation__invalid_variation_key(self):
        ...
        self.assertFalse(
            self.decision_service.set_forced_variation(
                self.project_config, 'test_experiment', 'test_user', 'variation_not_in_datafile',
            )
        )
        self.assertTrue(
    ...
    def test_set_forced_variation_when_called_to_remove_forced_variation(self):
        ...
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging:
            self.assertTrue(
                self.decision_service.set_forced_variation(self.project_config, 'group_exp_1', 'test_user', None)
            )
        mock_decision_service_logging.debug.assert_called_once_with(
    ...
    def test_get_forced_variation_with_none_set_for_user(self):
        ...
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging:
            self.assertIsNone(
                self.decision_service.get_forced_variation(self.project_config, 'test_experiment', 'test_user')
            )
        mock_decision_service_logging.debug.assert_called_once_with(
    ...
    def test_get_whitelisted_variation__user_in_forced_variation(self):
        ...
        mock_decision_service_logging.info.assert_called_once_with('User "user_1" is forced in variation "control".')
    ...
    def test_get_whitelisted_variation__user_in_invalid_variation(self):
        ...
        experiment = self.project_config.get_experiment_from_key('test_experiment')
        with mock.patch(
            'optimizely.project_config.ProjectConfig.get_variation_from_key', return_value=None,
    ...
    def test_get_variation__bucketing_id_provided(self):
        ...
        experiment = self.project_config.get_experiment_from_key('test_experiment')
        with mock.patch(
            'optimizely.decision_service.DecisionService.get_forced_variation', return_value=None,
        ), mock.patch(
            ...
            self.decision_service.get_variation(
                ...
                experiment,
                'test_user',
                {'random_key': 'random_value', '$opt_bucketing_id': 'user_bucket_value',},
            )
    ...
    def test_get_variation__user_has_stored_decision(self):
        ...
        experiment = self.project_config.get_experiment_from_key('test_experiment')
        with mock.patch(
            'optimizely.decision_service.DecisionService.get_whitelisted_variation', return_value=None,
        ) as mock_get_whitelisted_variation, mock.patch(
            'optimizely.decision_service.DecisionService.get_stored_variation',
            return_value=entities.Variation('111128', 'control'),
        ) as mock_get_stored_variation, mock.patch(
            ...
            'optimizely.user_profile.UserProfileService.lookup',
            return_value={'user_id': 'test_user', 'experiment_bucket_map': {'111127': {'variation_id': '111128'}},},
        ) as mock_lookup, mock.patch(
            'optimizely.user_profile.UserProfileService.save'
        ) as mock_save:
            self.assertEqual(
        ...
        mock_get_stored_variation.assert_called_once_with(
            self.project_config,
            experiment,
            user_profile.UserProfile('test_user', {'111127': {'variation_id': '111128'}}),
        )
        self.assertEqual(0, mock_audience_check.call_count)
        self.assertEqual(0, mock_bucket.call_count)
        self.assertEqual(0, mock_save.call_count)
    ...
    Also, stores decision if user profile service is available. """
        ...
        experiment = self.project_config.get_experiment_from_key('test_experiment')
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging, mock.patch(
            'optimizely.decision_service.DecisionService.get_whitelisted_variation', return_value=None,
        ) as mock_get_whitelisted_variation, mock.patch(
            'optimizely.decision_service.DecisionService.get_stored_variation', return_value=None,
        ) as mock_get_stored_variation, mock.patch(
            ...
            'optimizely.bucketer.Bucketer.bucket', return_value=entities.Variation('111129', 'variation'),
        ) as mock_bucket, mock.patch(
            'optimizely.user_profile.UserProfileService.lookup',
        ...
        mock_save.assert_called_once_with(
            {'user_id': 'test_user', 'experiment_bucket_map': {'111127': {'variation_id': '111129'}},}
        )
    ...
    def test_get_variation__user_profile_in_invalid_format(self):
        ...
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging, mock.patch(
            ...
            'optimizely.user_profile.UserProfileService.lookup', return_value='invalid_profile',
        ) as mock_lookup, mock.patch(
            'optimizely.user_profile.UserProfileService.save'
        ) as mock_save:
            self.assertEqual(
    ...
    def test_get_variation__user_profile_lookup_fails(self):
        ...
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging, mock.patch(
            ...
            'optimizely.user_profile.UserProfileService.lookup', side_effect=Exception('major problem'),
        ) as mock_lookup, mock.patch(
            'optimizely.user_profile.UserProfileService.save'
        ) as mock_save:
            self.assertEqual(
    ...
    def test_get_variation__user_profile_save_fails(self):
        ...
        with mock.patch.object(self.decision_service, 'logger') as mock_decision_service_logging, mock.patch(
            ...
            'optimizely.user_profile.UserProfileService.save', side_effect=Exception('major problem'),
        ) as mock_save:
            self.assertEqual(
                entities.Variation('111129', 'variation'),
                self.decision_service.get_variation(self.project_config, experiment, 'test_user', None),
...
class FeatureFlagDecisionTests(base.BaseTest):
    ...
    def test_get_variation_for_rollout__calls_bucket_with_bucketing_id(self):
        ...
        with mock.patch(
            ...
            self.assertEqual(
                ...
                self.decision_service.get_variation_for_rollout(
                    self.project_config, rollout, 'test_user', {'$opt_bucketing_id': 'user_bucket_value'},
                ),
            )
        ...
        # Check that bucket is called with correct parameters
        mock_bucket.assert_called_once_with(
            self.project_config, self.project_config.get_experiment_from_id('211127'), 'test_user', 'user_bucket_value',
        )
    ...
    def test_get_variation_for_feature__returns_variation_for_feature_in_experiment(self,):
        ...
        decision_patch = mock.patch(
            'optimizely.decision_service.DecisionService.get_variation', return_value=expected_variation,
        )
        with decision_patch as mock_decision, self.mock_decision_logger as mock_decision_service_logging:
            self.assertEqual(
                decision_service.Decision(expected_experiment, expected_variation, enums.DecisionSources.FEATURE_TEST,),
    ...
    the experiment in the rollout that the user is bucketed into. """
        ...
        get_variation_for_rollout_patch = mock.patch(
            'optimizely.decision_service.DecisionService.get_variation_for_rollout', return_value=expected_variation,
        )
        with get_variation_for_rollout_patch as mock_get_variation_for_rollout, self.mock_decision_logger as mock_decision_service_logging:
            self.assertEqual(
                expected_variation,
                self.decision_service.get_variation_for_feature(self.project_config, feature, 'test_user'),
            )
    ...
    feature's rollout even if the user is not bucketed into the feature's experiment. """
        ...
        expected_variation = self.project_config.get_variation_from_id('211127', '211129')
        with mock.patch(
            'optimizely.helpers.audience.is_user_in_experiment', side_effect=[False, True],
        ) as mock_audience_check, self.mock_decision_logger as mock_decision_service_logging, mock.patch(
            'optimizely.bucketer.Bucketer.bucket', return_value=expected_variation
        ):
            self.assertEqual(
                decision_service.Decision(expected_experiment, expected_variation, enums.DecisionSources.ROLLOUT,),
                self.decision_service.get_variation_for_feature(self.project_config, feature, 'test_user'),
            )
    ...
    user not in group and the feature is not part of a rollout. """
        ...
        with mock.patch(
            'optimizely.decision_service.DecisionService.get_experiment_in_group', return_value=None,
        ) as mock_get_experiment_in_group, mock.patch(
    ...
    def test_get_experiment_in_group(self):
        ...
        with mock.patch(
            ...
            self.assertEqual(
                experiment, self.decision_service.get_experiment_in_group(self.project_config, group, 'test_user'),
            )
