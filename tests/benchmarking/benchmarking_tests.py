...
class BenchmarkingTests(object):
    ...
    def activate_with_attributes(self, optimizely_obj, user_id):
        start_time = time.clock()
        variation_key = optimizely_obj.activate(
            'testExperimentWithFirefoxAudience', user_id, attributes={'browser_type': 'firefox'},
        )
        end_time = time.clock()
    ...
    def get_variation_with_attributes(self, optimizely_obj, user_id):
        start_time = time.clock()
        variation_key = optimizely_obj.get_variation(
            'testExperimentWithFirefoxAudience', user_id, attributes={'browser_type': 'firefox'},
        )
        end_time = time.clock()
        assert variation_key == 'variation'
    ...
    def track_with_attributes_and_revenue(self, optimizely_obj, user_id):
        start_time = time.clock()
        optimizely_obj.track(
            'testEventWithAudiences', user_id, attributes={'browser_type': 'firefox'}, event_value=666,
        )
        end_time = time.clock()
        return end_time - start_time
    ...
    def track_grouped_experiment_with_attributes(self, optimizely_obj, user_id):
        start_time = time.clock()
        optimizely_obj.track(
            'testEventWithMultipleExperiments', user_id, attributes={'browser_type': 'chrome'},
        )
        end_time = time.clock()
        return end_time - start_time
    ...
    def track_grouped_experiment_with_attributes_and_revenue(self, optimizely_obj, user_id):
        start_time = time.clock()
        optimizely_obj.track(
            'testEventWithMultipleExperiments', user_id, attributes={'browser_type': 'chrome'}, event_value=666,
        )
        end_time = time.clock()
        return end_time - start_time
...
def display_results(results_average, results_median):
  ...
  """
    ...
    table_headers = [
        'Test Name',
        '10 Experiment Average',
        '10 Experiment Median',
        '25 Experiment Average',
        '25 Experiment Median',
        '50 Experiment Average',
        '50 Experiment Median',
    ]
        ...
        for test_name, test_method in BenchmarkingTests.__dict__.iteritems():
            if callable(test_method):
                ...
                for i in xrange(ITERATIONS):
                    values.append(
                        1000 * test_method(BenchmarkingTests(), *test_data.get(test_name).get(experiment_count))
                    )
