...
class OptimizelyFactoryTest(base.BaseTest):
    ...
    def test_default_instance_should_create_http_config_manager_with_the_same_components_as_the_instance(self, _):
    ...
    def test_default_instance_with_manager_should_take_valid_custom_config_manager(self, _):
        class CustomConfigManager(object):
            def get_config(self):
                pass

        custom_config_manager = CustomConfigManager()
        optimizely_instance = OptimizelyFactory.default_instance_with_config_manager(custom_config_manager)
        self.assertEqual(optimizely_instance.config_manager, custom_config_manager)
        # validate through validator
        self.assertTrue(validator.is_config_manager_valid(CustomConfigManager()))

    def test_custom_instance_should_set_input_values_when_sdk_key_polling_interval_and_blocking_timeout_are_given(
            self, _):
    ...
    def test_custom_instance_should_set_default_values_when_sdk_key_polling_interval_and_blocking_timeout_are_invalid(
            self, _):
        OptimizelyFactory.set_polling_interval(-50)
        OptimizelyFactory.set_blocking_timeout(-10)
        optimizely_instance = OptimizelyFactory.custom_instance('sdk_key', None, self.event_dispatcher,
    ...
    def test_custom_instance_should_take_event_processor_when_flush_interval_and_batch_size_are_set_valid(self, _):
    ...
    def test_custom_instance_should_take_event_processor_set_default_values_when_flush_int_and_batch_size_are_invalid(
            self, _):
    ...
    def test_custom_instance_should_assign_passed_components_to_both_the_instance_and_config_manager(self, _):
        optimizely_instance = OptimizelyFactory.custom_instance('sdk_key', None, self.event_dispatcher,
                                                                self.mock_client_logger, self.error_handler, False,
                                                                self.user_profile_service, None,
                                                                self.notification_center)
    ...
    def test_max_event_batch_size_should_log_error_message_and_return_none_when_invalid(self, _):
    ...
    def test_max_event_batch_size_should_not_log_error_message_when_valid_batch_size(self, _):
        self.assertEqual(OptimizelyFactory.set_batch_size(5, self.mock_client_logger), 5)
        self.mock_client_logger.assert_not_called()

    def test_max_event_flush_interval_should_log_error_message_and_return_none_when_invalid(self, _):
    ...
    def test_max_event_flush_interval_should_not_log_error_message_when_valid_flush_interval(self, _):
