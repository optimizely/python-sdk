...
class OdpManagerTest(base.BaseTest):
    ...
    def test_configurations_disable_odp(self):
        ...
        manager.identify_user({'fs_user_id': 'user1', 'email': 'test@example.com'})
    ...
    def test_identify_user_single_identifier_skipped(self):
        ...
        mock_logger.debug.assert_called_with('ODP identify event is not dispatched (only one identifier provided).')
    ...
    def test_identify_user_empty_values_not_counted(self):
        ...
        with mock.patch.object(event_manager, 'identify_user') as mock_identify_user:
            manager.identify_user({'fs_user_id': 'user1', 'email': '', 'phone': ''})
